...
BIAS_YUV2RGB_OPENCV = np.array([-16, -128, -128], dtype=np.float32)
MATRIX_YUV2RGB_OPENCV = np.array([
                            [1.164, 0,        1.596],
                            [1.164, -0.391, -0.813],
                            [1.164, 2.018, 0]], dtype=np.float32)


def nv12_to_rgb_planar(yuv, dc, dh, dw):
	...
	yuv444=np.zeros((dc, dh, dw), np.float32)
	yuv444[0,:,:]=y[:,:]
	yuv444[1,:,:]=u[:,:]
	yuv444[2,:,:]=v[:,:]
...
def yuv_to_rgb(args):
	...
	## src
	if (yuv_color_fmt == 2):
		rgb_data_planar = nv12_to_rgb_planar(yuv_data, chan, height, width)
	else:
		raise UserWarning("Unsupport YUV color format : %d, only NV12 [2]" % (yuv_color_fmt))
...
def init_param(args):
	parser = argparse.ArgumentParser(description="Transfer NV12 to RGB, only support uint8 data format")
	...
	parser.add_argument("-m", type=int, required=False, default=1,
		help="The cvtcolor method (default 1). 0: arithmetic;  1: bias[0,-128,-128], matrix;  2: bias[-16, -128, -128], matrix in opencv")
